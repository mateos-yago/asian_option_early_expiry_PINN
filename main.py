...
#             print(
#                 f"Epoch {epoch:5d} | "
#                 f"Total: {loss_dict['loss_total']:.4e} | "
#                 f"PDE: {loss_dict['loss_pde']:.4e} | "
...
#             'loss': float(loss_dict['loss_total'].item() if hasattr(loss_dict['loss_total'], 'item') else loss_dict['loss_total']),
#             # Keep any other losses if you like
#             'losses': {k: float(v.item() if hasattr(v, 'item') else v) for k, v in loss_dict.items()},
#         }
#         torch.save(ckpt, f"checkpoints/ckpt_epoch_{epoch}.pt")
#
#     # final save
#
#     os.makedirs('checkpoints', exist_ok=True)
